...
class Model:
  ...
  def encode_obs(self, obs):
    ...
    result = np.copy(obs).astype(float)/255.0
  ...
  for episode in range(num_episode):
    ...
    for t in range(max_episode_length):
      ...
      if render_mode:
        ...
        temp_mu = np.round(np.copy(mu*10000)).astype(int).tolist()
        temp_sigma = np.round(np.copy(np.exp(logvar/2.0)*10000)).astype(int).tolist()
